 ...
 * No game-state changes: it wraps the existing renderHull() function.
 */
(() => {
  ...
  style.textContent = `
    ...
    #hull-bar::before, #hull-bar-p2::before {
      ...
      font-size: 17px;
      line-height: 1;
      text-shadow: 0 0 7px rgba(0, 210, 255, .95);
      pointer-events: none;
    }
    ...
    #hull-label, #hull-label-p2 {
      ...
      color: #fff !important;
      text-shadow: 0 0 6px rgba(0,0,0,.95), 0 1px 2px rgba(0,0,0,.8) !important;
    }
    ...
    #hull-bar.health-critical, #hull-bar-p2.health-critical {
      border-color: rgba(255, 64, 64, .95) !important;
    }
    #hull-bar.health-critical::before, #hull-bar-p2.health-critical::before {
      color: #ff4d5d !important;
      text-shadow: 0 0 8px rgba(255, 50, 70, 1) !important;
    }
  ...
  function decorate(idx, pct, previousPct) {
    const bar = idx === 0 ? document.getElementById('hull-bar') : document.getElementById('hull-bar-p2');
    const fill = idx === 0 ? document.getElementById('hull-fill') : document.getElementById('hull-fill-p2');
    if (!bar || !fill) return;
  ...
  const original = window.renderHull;
  if (typeof original !== 'function') {
    console.warn('[Health HUD] renderHull() was not available yet.');
    return;
  }

  window.renderHull = function(idx) {
    original(idx);
    const p = window.state && window.state.players ? window.state.players[idx] : null;
    if (!p) return;
    const pct = Math.max(0, Math.round((p.hearts / p.maxHearts) * 100));
    decorate(idx, pct, previous[idx]);
    previous[idx] = pct;
  };

  if (window.state && Array.isArray(window.state.players)) {
    window.state.players.forEach((p, i) => {
      if (p) window.renderHull(i);
    });
  }
})();
