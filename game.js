...
const ASSET_PATHS = {
  ...
  aliens: [
    ...
    { src: 'assets/aliens/hard.png', health: 5 },
  ],
...
async function preloadAllAssets(onProgress) {
  ...
  ASSET_PATHS.aliens.forEach((a, i) => {
    jobs.push(track(loadImage(a.src).then(img => { Images.aliens[i] = img; })));
  });
...
function pickAlienTier() {
  const available = alienTiersAvailable();
...
function maybeSpawn(dt, now) {
  ...
  if (roll < 0.42) {
    ...
    const tier = pickAlienTier();
    const alienType = ASSET_PATHS.aliens[tier];
    state.aliens.push({
      ...
      hp: alienType.health, maxHp: alienType.health
    });
  } else if (roll < 0.85) {
...
function update(dt, now) {
  ...
  state.players.forEach(p => {
    ...
    // Previous gameplay: the ship fires automatically at its normal fire rate.
    // Power-ups are collected by physically touching them; no power button is required.
  ...
  state.bullets.forEach(b => {
    state.aliens.forEach(a => {
      if (!b.dead && !a.dead && rectsOverlap(b, a)) {
        b.dead = true;
        a.hp -= 1;
        if (a.hp <= 0) { a.dead = true; addScoreOrCoins(5); Sound.explosion(); }
        else Sound.rockHit();
      }
    });
